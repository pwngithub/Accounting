...
# Ensure numeric conversion for all financial columns
...
net_profit = filtered_df[profit_cols].sum().sum() if profit_cols else total_income - total_expense

col1, col2, col3 = st.columns(3)
col1.metric("Total Income", f"${total_income:,.2f}")
col2.metric("Total Expenses", f"${total_expense:,.2f}")
if total_income:
    delta = (net_profit / total_income * 100)
    col3.metric("Net Profit", f"${net_profit:,.2f}", delta=f"{delta:.2f}%")
else:
